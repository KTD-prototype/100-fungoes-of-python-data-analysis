...
#########################################
# solve total logistics network problem #
#########################################
製品 = list('AB')
需要地 = list('PQ')
工場 = list('XY')
...
tbdi = pd.DataFrame(((j, k) for j in 需要地 for k in 工場), columns=[
                    '需要地', '工場'])
tbdi['輸送費'] = [1, 2, 3, 1]
print(tbdi)

# demand table
tbde = pd.DataFrame(((j, i) for j in 需要地 for i in 製品), columns=[
                    '需要地', '製品'])
tbde['需要'] = [10, 10, 20, 20]
print(tbde)

# production table
tbfa = pd.DataFrame(((k, l, i, 0, np.inf) for k, nl in zip(工場, レーン) for l in range(
    nl) for i in 製品), columns=['工場', 'レーン', '製品', '下限', '上限'])
tbfa['生産費'] = [1, np.nan, np.nan, 1, 2, np.nan, 5, 3]
